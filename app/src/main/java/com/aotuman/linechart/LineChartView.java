...
//    private boolean isInitialized = false;
    ...
    private void setupView(Context context){
        ...
        axisPaint.setColor(Color.parseColor("#ECF1F5"));

        // 画X轴文字
        axisTextPaint = new TextPaint();
        axisTextPaint.setTextSize(mYAxisFontSize);
        axisTextPaint.setAntiAlias(true);
//        axisTextPaint.setTextAlign(TextPaint.Align.LEFT);
        axisTextPaint.setColor(Color.parseColor("#142333"));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mHeight =heightSize;
        if(mXAxis == null){
            Log.e(TAG,"mWidth:"+mWidth+",mHeight:"+mHeight +"mXAxis:"+mXAxis);
            return;
        }
        //宽度通过数组长度计算
...
//            canvas.drawText(mXAxis.get(i), xfloat, 5 + mYAxisFontSize, axisTextPaint);
            xPoints[i] = (int) (xfloat+xTextWidth);

            // 画竖线
//            float xvfloat =  i * mxInterval -mLeftInterval/2;
//            canvas.drawLine(xvfloat,0, xvfloat,
//                    mHeight*dividerCount, axisPaint);
...
//        linePoints = new Point[dataList.size()];
    ...
    private void initAnim() {
        if (mXAxis != null) {
            valueAnimator = ValueAnimator.ofFloat(0f, 1f).setDuration(mXAxis.size() * 150);
